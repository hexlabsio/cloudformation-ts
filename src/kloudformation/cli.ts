...
async function setEnvsForStacks(stacks: string[], region: string): Promise<{ [key: string]: string; }> {
  try {
    const envs: Array<{ key: string, value: string; }[]> = await Promise.all(stacks.map(async (stack) => {
      ...
      console.log(chalk.green('Searching for stacks matching ' + searchString));
      const matchedStack = (cfStacks.Stacks ?? []).find(it => !!it.StackName.match(searchString) || !!it.StackId?.match(searchString));
      if (matchedStack) {
        console.log(chalk.green('Matched ' + matchedStack.StackId));
        return (matchedStack.Outputs ?? []).map(output => ({ key: output.OutputKey!, value: output.OutputValue! }));
      }
